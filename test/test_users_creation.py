...
class TestUserCreation:
    ...
    def test_login(self, API_URL, sure_user_exists, get_user_by_email, testuser_data):
        ...
        assert type(resp.json()["user"]["id"]) is int
        assert type(resp.json()["token"]) is str

        # check token column
        user = get_user_by_email(testuser_data['email'])
        print("user: {}", user)
        assert user.token == resp.json()["token"]

        # check expires date is in future
        now = datetime.now()
        expires = datetime.strptime(resp.json()["token_expires"], '%Y-%m-%d %H:%M:%S')
        assert now < expires
